...
describe('Cinema', function () {
  ...
  it('should be able to filter films by genre', function () {
    const actual = cinema.getFilmByGenre("sci-fi");
    assert.deepStrictEqual(actual, bladeRunner);
  });
  it('should be able to check whether there are some films from a particular year', function (){
    const actual = cinema.hasFilmByYear(2018);

    assert.deepStrictEqual(actual, true);
  });

  it('should be able to check whether there are no films from a particular year', function (){
    const actual = cinema.hasFilmByYear(2010);
    assert.deepStrictEqual(actual, false);
  });

  it('should be able to check whether all films are over a particular length', function(){
    const actual = cinema.filmOverDuration(90);
    assert.deepStrictEqual(actual, true);
  });

  it('should be able to calculate total running time of all films', function() {
    const actual = cinema.totalRunningOfAllFilms();
    assert.deepStrictEqual(actual, 622);
  });


  it('should be able to filter films by genre', function () {
    const actual = cinema.getFilmByProperty("genre", 'sci-fi');
    assert.deepStrictEqual(actual[0], bladeRunner);
  });
